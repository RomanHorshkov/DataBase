...
{
    if (mdb_rc == MDB_SUCCESS)  return 0;
    if (mdb_rc == MDB_NOTFOUND) return -ENOENT;
    return -EIO;
}
                         ...
                         const uint8_t resource[DB_ID_SIZE])
{
    ...
    if (mdb_put(txn, DB->db_acl_fwd, &fk, &fv, 0) != MDB_SUCCESS) return -EIO;
    ...
    if (mdb_put(txn, DB->db_acl_by_res, &rk, &rv, 0) != MDB_SUCCESS) return -EIO;
                           ...
                           const uint8_t resource[DB_ID_SIZE])
{
    ...
    if (rc == 0) return 0; else if (rc != -ENOENT) return rc;
...
static int db_get_data_meta(uint8_t data_id[DB_ID_SIZE], DataMeta *out_meta)
{
    ...
    MDB_txn *txn;
    if (mdb_txn_begin(DB->env, NULL, MDB_RDONLY, &txn) != MDB_SUCCESS) return -EIO;
    ...
    if (v.mv_size != sizeof(DataMeta)) { mdb_txn_abort(txn); return -EIO; }
...
static int db_user_set_role(uint8_t userId[DB_ID_SIZE], user_role_t role)
{
    ...
    UserPacked up; memcpy(&up, v.mv_data, sizeof up); up.role = role;
    MDB_val nv = { .mv_size = sizeof up, .mv_data = (void *) &up };
    if (mdb_put(txn, DB->db_user, &k, &nv, 0) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
    if (mdb_txn_commit(txn) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
    return 0;
}
...
int db_open(const char *root_dir, uint64_t mapsize_bytes)
{
    ...
    if (erc != 0) return erc;  /* already -EIO */
    ...
    if (mdb_env_create(&DB->env) != MDB_SUCCESS) { free(DB); DB = NULL; return -EIO; }

    mdb_env_set_maxdbs(DB->env, 32);
    mdb_env_set_mapsize(DB->env, (size_t) mapsize_bytes);

    char metadir[2048]; snprintf(metadir, sizeof metadir, "%s/meta", root_dir);
    if (mdb_env_open(DB->env, metadir, 0, 0770) != MDB_SUCCESS) {
        mdb_env_close(DB->env); free(DB); DB = NULL; return -EIO;
    }
    ...
    if (mdb_dbi_open(txn, DB_USER,          MDB_CREATE,                                 &DB->db_user        ) != MDB_SUCCESS) goto fail;
    if (mdb_dbi_open(txn, DB_USER_EMAIL2ID, MDB_CREATE,                                 &DB->db_user_email2id) != MDB_SUCCESS) goto fail;
    if (mdb_dbi_open(txn, DB_DATA_META,     MDB_CREATE,                                 &DB->db_data_meta   ) != MDB_SUCCESS) goto fail;
    if (mdb_dbi_open(txn, DB_SHA2DATA,      MDB_CREATE,                                 &DB->db_sha2data    ) != MDB_SUCCESS) goto fail;

    /* ACLs: forward (presence sentinel) + reverse (dupsort, dupfixed) */
    if (mdb_dbi_open(txn, DB_ACL_FWD,       MDB_CREATE,                                 &DB->db_acl_fwd     ) != MDB_SUCCESS) goto fail;
    if (mdb_dbi_open(txn, DB_ACL_BY_RES,    MDB_CREATE | MDB_DUPSORT | MDB_DUPFIXED,    &DB->db_acl_by_res  ) != MDB_SUCCESS) goto fail;

    if (mdb_txn_commit(txn) != MDB_SUCCESS) { mdb_txn_abort(txn); goto fail_env; }
    return 0;
...
int db_add_user(const char email[EMAIL_MAX_LEN], uint8_t out_id[DB_ID_SIZE])
{
    ...
    /* Generate unique id */
    uint8_t id[DB_ID_SIZE];
    do { id128_rand(id); } while (db_user_find_by_id(id, NULL) == 0);
    ...
    if (mrc == MDB_KEYEXIST) { mdb_txn_abort(txn); if (out_id) memcpy(out_id, id, DB_ID_SIZE); return -EEXIST; }
...
int db_user_find_by_id(const uint8_t id[DB_ID_SIZE], char out[EMAIL_MAX_LEN])
{
    ...
    if (out) memcpy(out, ((UserPacked *) v.mv_data)->email, EMAIL_MAX_LEN);
    mdb_txn_abort(txn);
    return 0;
}
                                       ...
                                       const char email[EMAIL_MAX_LEN])
{
    ...
    /* Resolve recipient */
    {
        ...
        if (frc != 0) return frc; /* -ENOENT / -EIO / -EINVAL */
    }

    /* Role check stays (coarse global permissions) */
    {
        user_role_t owner_role = 0; int prc = db_user_get_role(owner, &owner_role);
        if (prc == -ENOENT) return -ENOENT;
        if (prc != 0) return -EIO;
        if (owner_role != USER_ROLE_PUBLISHER
            && owner_role != USER_ROLE_VIEWER) return -EPERM;
    }

    /* One RW transaction for existence check + ACL checks + grant */
    MDB_txn *txn; if (mdb_txn_begin(DB->env, NULL, 0, &txn) != MDB_SUCCESS) return -EIO;
    ...
    /* Ensure data exists */
    {
        ...
        if (mrc != MDB_SUCCESS || v.mv_size != sizeof(DataMeta)) { mdb_txn_abort(txn); return -EIO; }
    }

    /* Sharer must have any of O/S/U on this data */
    if (acl_has_any_txn(txn, owner, data_id) != 0) { mdb_txn_abort(txn); return -EPERM; }

    /* Presence in 'U' (view) namespace for recipient */
    if (acl_grant_txn(txn, target_user_id, ACL_RTYPE_USER, data_id) != 0) { mdb_txn_abort(txn); return -EIO; }
...
int db_upload_data_from_fd(uint8_t owner[DB_ID_SIZE], int src_fd, const char *mime, uint8_t out_data_id[DB_ID_SIZE])
{
    ...
    Sha256 digest; size_t total = 0;
    if (crypt_store_sha256_object_from_fd(DB->root, src_fd, &digest, &total) != 0) return -EIO;
    ...
    if (mrc == MDB_SUCCESS && sv.mv_size == DB_ID_SIZE)
    {
        ...
        if (acl_grant_txn(txn, owner, ACL_RTYPE_OWNER, data_id) != 0) { mdb_txn_abort(txn); return -EIO; }
        if (mdb_txn_commit(txn) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
    ...
    else if (mrc == MDB_NOTFOUND)
    {
        ...
        DataMeta mp; memset(&mp, 0, sizeof mp);
        mp.ver        = 1;
        ...
        if (mdb_put(txn, DB->db_data_meta, &mk, &mv, 0) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
        if (mdb_put(txn, DB->db_sha2data,  &sk, &siv, 0) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
        if (acl_grant_txn(txn, owner, ACL_RTYPE_OWNER, data_id) != 0)   { mdb_txn_abort(txn); return -EIO; }
...
int db_resolve_data_path(uint8_t data_id[DB_ID_SIZE], char *out_path, unsigned long out_sz)
{
    ...
    char    hex[65];
    Sha256  d; memcpy(d.b, meta.sha, 32);
...
int db_user_set_role_viewer   (uint8_t userId[DB_ID_SIZE]) { return db_user_set_role(userId, USER_ROLE_VIEWER); }
int db_user_set_role_publisher(uint8_t userId[DB_ID_SIZE]) { return db_user_set_role(userId, USER_ROLE_PUBLISHER); }
...
int db_user_list_all(uint8_t* out_ids, size_t* inout_count_max)
{
    ...
    MDB_txn* txn; if (mdb_txn_begin(DB->env, NULL, MDB_RDONLY, &txn) != MDB_SUCCESS) return -EIO;
    MDB_cursor* cur; if (mdb_cursor_open(txn, DB->db_user, &cur) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
    ...
    {
        if (v.mv_size != sizeof(UserPacked)) continue;
        const UserPacked* up = (const UserPacked*)v.mv_data;
        if (n < cap && out_ids) memcpy(out_ids + n*DB_ID_SIZE, up->id, DB_ID_SIZE);
        n++;
    }
...
{
    if (!actor || !data_id) return -EINVAL;
    ...
    DataMeta meta = {0};
    {
        ...
        if (rc == MDB_NOTFOUND) { mdb_txn_abort(txn); return -ENOENT; }
        if (rc != MDB_SUCCESS || v.mv_size != sizeof(DataMeta)) { mdb_txn_abort(txn); return -EIO; }
    ...
    /* Owner check: actor must have presence in 'O' */
    {
        ...
        if (rc == -ENOENT) { mdb_txn_abort(txn); return -EPERM; }
    ...
    {
        char rt = rtypes[i];
        uint8_t rkey[17];  acl_rev_key(rkey, data_id, rt);

        MDB_cursor* cur = NULL;
        if (mdb_cursor_open(txn, DB->db_acl_by_res, &cur) != MDB_SUCCESS) { mdb_txn_abort(txn); return -EIO; }
        ...
        for (;;)
        {
            ...
            if (rc == MDB_NOTFOUND) break;  /* no more dupset for this rtype */
            if (rc != MDB_SUCCESS) { mdb_cursor_close(cur); mdb_txn_abort(txn); return -EIO; }

            if (v.mv_size == DB_ID_SIZE)
            {
                uint8_t principal[DB_ID_SIZE]; memcpy(principal, v.mv_data, DB_ID_SIZE);
                /* delete forward pair */
                uint8_t fkey[33]; acl_fwd_key(fkey, principal, rt, data_id);
                MDB_val fk = { .mv_size = sizeof fkey, .mv_data = fkey };
