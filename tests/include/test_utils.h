...
typedef struct { const char* name; test_fn fn; } TU_Test;
...
/* ------------------------------ Fixture ---------------------------------- */
typedef struct { char root[PATH_MAX]; } Ctx;
...
int  tu_io_set_files(const char* out_path, const char* err_path); /* owns files */
...
#define EXPECT_TRUE(cond) do{ if(!(cond)) tu_failf(__FILE__,__LINE__,"EXPECT_TRUE(%s) failed", #cond); }while(0)
#define EXPECT_EQ_INT(a,b) do{ long _A=(long)(a), _B=(long)(b); if(_A!=_B) tu_failf(__FILE__,__LINE__,"EXPECT_EQ_INT(%s=%ld, %s=%ld)", #a,_A,#b,_B); }while(0)
#define EXPECT_EQ_SIZE(a,b) do{ size_t _A=(size_t)(a), _B=(size_t)(b); if(_A!=_B) tu_failf(__FILE__,__LINE__,"EXPECT_EQ_SIZE(%s=%zu, %s=%zu)", #a,_A,#b,_B); }while(0)
#define EXPECT_EQ_RC(rc,exp) do{ int _r=(rc), _e=(exp); if(_r!=_e) tu_failf(__FILE__,__LINE__,"rc=%d(%s) expected %d(%s)", _r, tu_errname(_r), _e, tu_errname(_e)); }while(0)
#define EXPECT_EQ_ID(a,b) do{ if(memcmp((a),(b),16)!=0){ char _ha[33],_hb[33]; tu_hex16(_ha,(a)); tu_hex16(_hb,(b)); tu_failf(__FILE__,__LINE__,"IDs differ: %s != %s", _ha,_hb);} }while(0)
