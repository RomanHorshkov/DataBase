...
#define EXPECT_EQ_INT(a, b)                                                                \
    ...
    {                                                                                      \
        ...
        if(_A != _B)                                                                       \
            tu_failf(__FILE__, __LINE__, "EXPECT_EQ_INT(%s=%ld, %s=%ld)", #a, _A, #b, _B); \
    } while(0)
#define EXPECT_EQ_SIZE(a, b)                                                                \
    ...
    {                                                                                       \
        ...
        if(_A != _B)                                                                        \
            tu_failf(__FILE__, __LINE__, "EXPECT_EQ_SIZE(%s=%zu, %s=%zu)", #a, _A, #b, _B); \
    } while(0)
#define EXPECT_EQ_RC(rc, exp)                                                                                  \
    ...
    {                                                                                                          \
        int _r = (rc), _e = (exp);                                                                             \
        if(_r != _e)                                                                                           \
            tu_failf(__FILE__, __LINE__, "rc=%d(%s) expected %d(%s)", _r, tu_errname(_r), _e, tu_errname(_e)); \
    } while(0)
