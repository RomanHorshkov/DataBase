...
static inline FILE* SERR(void)
{
    return g_err ? g_err : stderr;
}
...
void tu_io_reset(void)
{
    ...
    g_out = NULL;
    g_err = NULL; /* back to stdout/stderr */
}

int tu_io_set(FILE* out, FILE* err)
{
    /* no ownership */
    if(g_out_owned || g_err_owned)
    {
        tu_io_reset();
    }
    g_out = out;
