...
int t_open_creates_layout(void){
    ...
    tu_teardown_store(&ctx); return 0;
}
...
int t_upload_requires_publisher(void){
    ...
    char ea[EMAIL_MAX_LEN]; snprintf(ea,sizeof ea,"%s","a@x");
    int fd = tu_make_blob("./.tmp_blob.dcm","shared-seed-001"); EXPECT_TRUE(fd>=0);
...
int t_dedup_same_sha(void){
    ...
    int fd = tu_make_blob("./.tmp_blob2.dcm","same-content"); EXPECT_TRUE(fd>=0);
...
int t_share_by_email(void){
    ...
    int fd = tu_make_blob("./.tmp_blob3.dcm","to-share"); EXPECT_TRUE(fd>=0);
    uint8_t D[DB_ID_SIZE]={0};
    EXPECT_EQ_RC(db_upload_data_from_fd(A, fd, "application/dicom", D), 0);
...
int t_resolve_path_points_to_object(void){
    ...
    int fd = tu_make_blob("./.tmp_blob4.dcm","path-check"); EXPECT_TRUE(fd>=0);
    uint8_t D[DB_ID_SIZE]={0};
    EXPECT_EQ_RC(db_upload_data_from_fd(A, fd, "application/dicom", D), 0);
...
int t_share_requires_relationship(void){
    ...
    /* B uploads a blob */
    int fd = tu_make_blob("./.tmp_blob5.dcm","owned-by-B"); EXPECT_TRUE(fd>=0);
    uint8_t D[DB_ID_SIZE]={0};
...
int t_owner_delete_cascade(void){
    ...
    int fd = tu_make_blob("./.tmp_blob6.dcm","delete-me"); EXPECT_TRUE(fd>=0);
    uint8_t D[DB_ID_SIZE]={0};
...
int t_dedup_multi_owner_can_delete(void){
    ...
    int fd = tu_make_blob("./.tmp_blob7.dcm","same-bits" ); EXPECT_TRUE(fd>=0);
    uint8_t D1[DB_ID_SIZE]={0}, D2[DB_ID_SIZE]={0};
