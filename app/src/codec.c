...
int pr_uuid(const MDB_val* in, FILE* out)
{
    ...
    const uint8_t* b = (const uint8_t*)in->mv_data;
...
// {
//     const acl_fwd_key_t*                 k = (const acl_fwd_key_t*)obj;
...
// {
//     const acl_rev_key_t*                 k = (const acl_rev_key_t*)obj;
//     static _Thread_local unsigned char buf[DB_ID_SIZE + 1 + DB_ID_SIZE];
//     unsigned char*                     p = buf;
//     memcpy(p, k->resource.b, DB_ID_SIZE);
//     p    += DB_ID_SIZE;
...
//     const uint8_t* p = (const uint8_t*)in->mv_data;
//     const uint8_t* a = p;
//     const uint8_t* b = p + DB_ID_SIZE + 1;
//     for(size_t i = 0; i < DB_ID_SIZE; i++)
//         fprintf(out, "%02x", a[i]);
