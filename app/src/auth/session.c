...
{
    if (!user_id || !access_token || !refresh_token) { errno = EINVAL; return -1; }
...
int session_validate_access(const char* token, uint8_t out_user_id[16], uint64_t now_sec)
{
    if (!token || !out_user_id) { errno = EINVAL; return -1; }
...
int session_rotate_refresh(const char* refresh_token, char new_access[SESSION_STR_MAX],
                        char new_refresh[SESSION_STR_MAX], uint64_t now_sec)
{
    if (!refresh_token || !new_access || !new_refresh) { errno = EINVAL; return -1; }
...
{
    if (!user_id) { errno = EINVAL; return -1; }
...
{
    if (!any_token) { errno = EINVAL; return -1; }
