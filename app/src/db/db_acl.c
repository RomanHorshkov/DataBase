...
static void acl_fwd_key(uint8_t out[33], uint8_t principal[DB_UUID_SIZE],
                        char rel, uint8_t resource[DB_UUID_SIZE]);

/* resource(16) | rel(1) => 17 bytes (dupset key for principals) */
static void acl_rev_key(uint8_t out[17], uint8_t resource[DB_UUID_SIZE],
                        char rel);

static inline void fwd_key(uint8_t out[33], uint8_t principal[DB_UUID_SIZE],
                           uint8_t rel, uint8_t resource[DB_UUID_SIZE])
{
    acl_fwd_key(out, principal, (char)rel, resource);
}

static inline void rev_key(uint8_t out[17], uint8_t resource[DB_UUID_SIZE],
                           uint8_t rel)
{
    acl_rev_key(out, resource, (char)rel);
}

static int put_forward(MDB_txn* txn, uint8_t principal[DB_UUID_SIZE],
                       uint8_t rel, uint8_t resource[DB_UUID_SIZE]);

static int put_reverse(MDB_txn* txn, uint8_t resource[DB_UUID_SIZE],
                       uint8_t rel, uint8_t principal[DB_UUID_SIZE]);

static void del_forward(MDB_txn* txn, uint8_t principal[DB_UUID_SIZE],
                        uint8_t rel, uint8_t resource[DB_UUID_SIZE]);

static void del_reverse(MDB_txn* txn, uint8_t resource[DB_UUID_SIZE],
                        uint8_t rel, uint8_t principal[DB_UUID_SIZE]);

static int has_forward(MDB_txn* txn, uint8_t principal[DB_UUID_SIZE],
                       uint8_t rel, uint8_t resource[DB_UUID_SIZE]);
    ...
    for(size_t i = 0; i < 3; ++i)
    {
        ...
        for(;;)
        {
            ...
            if(v.mv_size == DB_UUID_SIZE)
            {
                ...
                uint8_t fkey[33];
                acl_fwd_key(fkey, (uint8_t*)v.mv_data, rel, resource);
