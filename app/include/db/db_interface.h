 ...
 * @return 0 on insertion, -EEXIST if already existed, -EINVAL bad input, -EIO
 * DB error.
 */
 ...
 * @return 0 on success, -ENOENT if user missing, -EINVAL if bad role, -EIO on
 * DB error.
 */
 ...
 * @return 0 on success, -ENOENT if meta missing, -EINVAL bad args, -EIO on path
 * error.
 */
 ...
 * @return 0 on success, -EEXIST if content existed (id returned), -EPERM if not
 * publisher, -ENOENT if owner not found, -EINVAL bad args, -EIO on error.
 */
...
/*
 * int db_revoke_data_from_user_email(uint8_t owner[DB_ID_SIZE], uint8_t
 * data_id[DB_ID_SIZE], const char email[DB_EMAIL_MAX_LEN]); int
 * db_revoke_data_from_user_id(uint8_t owner[DB_ID_SIZE], uint8_t
 * data_id[DB_ID_SIZE], const uint8_t user_id[DB_ID_SIZE]);
 */
