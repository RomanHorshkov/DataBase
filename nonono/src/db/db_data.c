...
static inline void write_data_meta(void *dst, const Sha256 *digest,
                                   ...
                                   uint64_t      created_at,
                                   const uint8_t owner[DB_UUID_SIZE])
{
    ...
    memcpy(m->owner, owner, DB_UUID_SIZE);
}
...
int db_data_get_meta(uint8_t data_id[DB_UUID_SIZE], DataMeta *out_meta)
{
    ...
    MDB_val k   = {.mv_size = DB_UUID_SIZE, .mv_data = data_id};
    MDB_val v   = {0};
                        ...
                        const char *mime, uint8_t out_data_id[DB_UUID_SIZE])
{
    ...
    uint8_t  data_id[DB_UUID_SIZE] = {0};
...
retry_chunk:
    ...
    MDB_val shav = {.mv_size = DB_UUID_SIZE, .mv_data = NULL};
    ...
    memcpy(shav.mv_data, data_id, DB_UUID_SIZE);
    ...
    if(out_data_id) memcpy(out_data_id, data_id, DB_UUID_SIZE);
    return 0;
}
                   ...
                   const uint8_t data_id[DB_UUID_SIZE])
{
    ...
    /* drop lookups */
    {
        ...
        MDB_val mk = {.mv_size = DB_UUID_SIZE, .mv_data = (void *)data_id};
